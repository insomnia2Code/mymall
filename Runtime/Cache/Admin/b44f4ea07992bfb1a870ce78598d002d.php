<?php if (!defined('THINK_PATH')) exit(); if(is_array($tree)): $i = 0; $__LIST__ = $tree;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$list): $mod = ($i % 2 );++$i;?><dl class="cate-item">
		<dt class="cf">
			<form action="<?php echo U('edit');?>" method="post">
				...
				<div class="order"><input type="text" name="listorder" class="text input-mini" value="<?php echo ($list["listorder"]); ?>"></div>
				<div class="order"><?php echo ($list['is_display']?'是':'否'); ?></div>
				<div class="name">
		...
		<?php if(!empty($list['_'])): ?><dd>
				<?php echo R('GoodsCategory/tree', array($list['_']));?>
			</dd><?php endif; ?>
